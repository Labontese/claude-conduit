...
server.tool('conduit_search_tools', 'Search registered tools by intent. Returns name and description only — no schemas.', {
...
server.tool('conduit_describe_tool', 'Get full schema for a specific tool by name.', { name: z.string().describe('Tool name') }, async ({ name }) => {
    ...
    return {
        content: [
            {
                type: 'text',
                text: JSON.stringify({ name: tool.name, description: tool.description, inputSchema: tool.inputSchema }, null, 2),
            },
        ],
    };
});
server.tool('conduit_execute_tool', 'Execute a registered tool by name with given arguments.', {
...
server.tool('conduit_wrap_request', 'Optimize an Anthropic API request with cache breakpoints. Returns optimized request + token savings metadata.', {
    ...
    const wrapped = cacheOrchestrator.wrapRequest(request, disable);
    return { content: [{ type: 'text', text: JSON.stringify(wrapped, null, 2) }] };
});
server.tool('conduit_report', 'Get token usage and cost report for current session.', {
    session_id: z.string().optional(),
    format: z.enum(['json', 'markdown']).optional().default('markdown'),
}, async ({ session_id, format }) => {
    const report = obs.getSessionReport(session_id);
    const text = format === 'markdown' ? obs.formatReport(report) : JSON.stringify(report, null, 2);
    return { content: [{ type: 'text', text }] };
});
server.tool('conduit_explain', 'Human-readable explanation of what conduit optimized this session.', { request_id: z.string().optional() }, async () => {
    ...
    return {
        content: [
            {
                type: 'text',
                text: `conduit has processed ${report.requestCount} request(s) this session.\n` +
